...
class RnRongtaPrinterModule(reactContext: ReactApplicationContext) :
  NativeRnRongtaPrinterSpec(reactContext) {
  ...
  companion object {
    const val NAME = "RnRongtaPrinter"
  }
    ...
    fun connectPrinter(ip:String,port:Int) {
        ...
        try {
            ...
            Log.d("RongtaPrinterModule","Connected Printer from wifi");
        } catch (e: Exception) {
    ...
    override fun printerObserverCallback(arg0: PrinterInterface<Any>?, arg1: Int) {
        when (arg1) {
            CommonEnum.CONNECT_STATE_SUCCESS -> {
                ...
                Log.d("RongtaPrinterModule","Connected Printer")
            }
            CommonEnum.CONNECT_STATE_INTERRUPTED -> {
                println("Disconnected Printer")
                Log.d("RongtaPrinterModule","Disconnected Printer")
            }
        }
    }
    ...
    fun initPrinter(IP: String, PORT: Int, promise: Promise) {
        try {
            ...
            Log.d("RongtaPrinterModule","Init printer success");
            promise.resolve("connected")
        }
        catch(e: Exception) {
            Log.d("RongtaPrinterModule","Init printer failed");
    ...
    public fun doDisConnect(promise: Promise) {
        if (rtPrinter?.printerInterface != null) {
            try {
                ...
                Log.d("RongtaPrinterModule","rongta printer disconnected successfully");
    ...
    @ReactMethod
    fun tscPrint(imageUrl: String, hlength: Int?, vlength: Int?, promise: Promise) {
        Thread {
            try {
                // Default Values
                val width = hlength ?: 95 // mm
                val height = vlength ?: 250 // mm
                ...
                rtPrinter?.let { printer ->
                    Log.d("RongtaPrinterModule", printer.toString())
    ...
    fun print(text: String) {
        try {
            ...
            Log.d("RongtaPrinterModule","Printer data written successfully")
        } catch (e: Exception) {
